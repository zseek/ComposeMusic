...
class MusicRepositoryImpl(private val dao: MusicDao) : MusicRepository {
    ...
    override suspend fun updateLoadingStatus(songID: Long, isLoading: Boolean) =
        dao.updateLoadingStatus(songID, isLoading)

    override suspend fun updateDuration(songID: Long, duration: Long) =
        dao.updateDuration(songID, duration)
